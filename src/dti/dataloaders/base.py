...
def bacpi_create_ijbonddict(mol):
...
def bacpi_atom_features(atoms, i_jbond_dict, radius):
...
def update_bacpi_dict(smiles):
    mol = Chem.MolFromSmiles(smiles)
    try: Chem.SanitizeMol(mol)
    except: pass 
    atoms = bacpi_create_atoms(mol)
    i_jbond_dict = bacpi_create_ijbonddict(mol)
    _ = bacpi_atom_features(atoms, i_jbond_dict, 2) 

def make_amino_ngram_dict(alphabets='ABCDEFGHIKLMNOPQRSTUVWXYZ', n=3):
    amino_ngrams = [''.join(x) for x in itertools.product(alphabets, repeat=n)]
    amino_ngrams.append('???')
    amino_ngrams = dict(zip(amino_ngrams, [i+1 for i in range(len(amino_ngrams))]))

    return amino_ngrams

def get_amino_ngrams(fasta, n, amino_ngram_dict=None):

    return [fasta[i:i+n] for i in range(len(fasta)-n+1)]
